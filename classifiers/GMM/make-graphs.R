...
for(m in 1:length(data_sets)) {
  ...
  for(n in 1:length(pids)) {
    ...
    # Generate plots
    meta_rupture_cutoff <- classifier_conf[['meta_rupture_cutoff']]
    baseline <- mean(df$stationary_median[which(df$event == "N")], na.rm=T)
    limit <- max(c(
      abs(min(c(df$stationary_median*1.5, meta_rupture_cutoff*1.5), na.rm=T)), 
      abs(max(c(df$stationary_median*1.5, meta_rupture_cutoff*1.5), na.rm=T))
    ), na.rm=T)
    plot <- base_plot + 
      geom_line(aes(x=time, y=stationary_median)) +
      geom_line(aes(x=time, y=GMM_prob), color="gray") +
      geom_line(aes(x=time, y=GMM_prob_spline), color="red") +
      ...
      annotate(geom="segment", x=-Inf, xend=Inf, y=meta_rupture_cutoff, yend=meta_rupture_cutoff, color="red", alpha=0.7) +
