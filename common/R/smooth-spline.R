...
if(length(args) == 4) {
  spars <- as.numeric(strsplit(args[4], ",")[[1]])
} else {
  spars <- rep.int(0, length(columns))
}
...
if(length(columns) != length(spars)) {
  stop("Need as many spars as columns")
}
...
# @param float spar The smoothing parameter
# @returns dataframe The modified dataframe
fit_spline <- function(group, fit_column, new_column_stem, spar=0) {
  x <- group$time
  y <- group[[fit_column]]
  w <- rep.int(1, length(y))
  w[is.na(y)] <- 0
  y[is.na(y)] <- 0
  spline <- smooth.spline(x, y, w=w, spar=spar)
...
fit_splines <- function(group, columns, column_stems, spars) {
  for(idx in 1:length(columns)) {
    group <- fit_spline(group, columns[idx], column_stems[idx], spars[idx])
  }
  return(group)
}
...
data <- ddply(data, .(data_set, particle_id), fit_splines, columns=columns, column_stems=column_stems, spars=spars)
